...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside 
        className={cn(
          "flex flex-col w-64 min-h-screen bg-[#0d0d10] border-r border-white/5 px-4 py-6 fixed top-0 left-0 z-30 transition-transform duration-300 ease-in-out",
      ...
      >
        ...
        <div className="flex items-center gap-3 px-2 mb-10">
          ...
          <span className="ml-auto text-[10px] font-medium text-violet-400 bg-violet-500/10 border border-violet-500/20 rounded-full px-2 py-0.5">
            Admin
          </span>
          
          {/* Mobile Close Button */}
          <button 
            className="md:hidden ml-1 p-1 text-zinc-500 hover:text-white"
            onClick={() => setIsOpen(false)}
          >
        ...
        <nav className="flex flex-col gap-1 flex-1">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={cn(isActive ? "text-violet-400" : "text-zinc-600")}>
                  {item.icon}
                </span>
                {item.label}
                {isActive && (
        ...
        <div className="mt-auto pt-6 border-t border-white/5">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-white truncate">Alice Hoffman</p>
              <p className="text-xs text-zinc-500 truncate">alice@nexus.io</p>
            </div>
